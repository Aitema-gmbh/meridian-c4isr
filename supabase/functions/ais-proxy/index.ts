...
// No mock data — commercial vessels require a live aisstream.io API key
...
serve(async (req) => {
  ...
  try {
    ...
    let source: "live" | "military-only" = "military-only";
    ...
    if (apiKey) {
      try {
        ...
        if (liveData.length > 0) {
          ...
          console.log("aisstream.io: no data received, returning military only");
        }
      } catch (e) {
        console.error("aisstream.io error:", e);
      }
    } else {
      console.log("No AISSTREAM_API_KEY set — returning military vessels only");
    }
